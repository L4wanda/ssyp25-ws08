...
class ExampleApp(QMainWindow):
    def __init__(self):
        ...
        self.scale_height_mie = self.spinBox_4.value()

        self.spinBox_2.valueChanged.connect(self.height_rayleigh_update)
        self.spinBox_4.valueChanged.connect(self.height_mie_update)
    ...
    def update_rayleigh(self, frame):
        ...
        color = SkydomeRenderer(False, self.latitude, self.longitude, self.width, self.height,
                                self.scale_height_rayleigh, self.scale_height_mie).render(minutes)
    ...
    def update_mie(self, frame):
        minutes = (frame * 10) % (24 * 60) # every 10 minutes
        color = SkydomeRenderer(True, self.latitude, self.longitude, self.width, self.height,
                                self.scale_height_rayleigh, self.scale_height_mie).render(minutes)
        color = np.array(color)
        color[color>1] = 1
        cur_hours = minutes // 60
    ...
    def update_sum(self, frame):
        ...
        color = (SkydomeRenderer(False, self.latitude, self.longitude, self.width, self.height,
                                 self.scale_height_rayleigh, self.scale_height_mie).render(minutes)
                 + SkydomeRenderer(True, self.latitude, self.longitude, self.width, self.height,
                                   self.scale_height_rayleigh, self.scale_height_mie).render(minutes))
        color = np.array(color)
        color[color>1] = 1
        cur_hours = minutes // 60
    ...
    def height_mie_update(self, value):
        self.scale_height_mie = value
